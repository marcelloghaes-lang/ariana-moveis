...
// =============================
// SELLER AUTH COMPATÍVEL
// Corrige login do portal seller e mantém /api/auth/login intacto.
// Permite login tanto por User(role=seller) quanto por Seller criado via solicitação.
// =============================
async function buildSellerSessionResponse(userDoc, sellerDoc = null) {
  let user = userDoc || null;
  let seller = sellerDoc || null;

  if (user && !seller) {
    if (user.sellerId) seller = await Seller.findOne({ sellerId: user.sellerId });
    if (!seller && user.email) seller = await Seller.findOne({ email: String(user.email).trim().toLowerCase() });
  }

  if (seller && user && !user.sellerId) {
  ...
  const token = signToken(user);
  const userObj = toJSON(user) || {};
  const sellerObj = toJSON(seller) || {};
  ...
  return {
    ...
    seller: {
      ...userObj,
      ...sellerObj,
      id: sellerObj.id || sellerObj._id || userObj.id || userObj._id,
      sellerId: sellerObj.sellerId || userObj.sellerId || null,
      name: sellerObj.displayName || sellerObj.storeName || userObj.name || '',
      displayName: sellerObj.displayName || userObj.name || '',
      storeName: sellerObj.storeName || sellerObj.displayName || userObj.name || '',
      email: sellerObj.email || userObj.email || '',
      role: 'seller',
      active: sellerObj.status ? !['rejected', 'blocked', 'inactive'].includes(String(sellerObj.status).toLowerCase()) : true,
      status: sellerObj.status || 'approved',
      payout: sellerObj.metadata?.payout || sellerObj.metadata?.pagarme || sellerObj.metadata?.recipient || null
    }
  };
}

async function validateSellerMetadataPassword(seller, password) {
  const meta = seller?.metadata && typeof seller.metadata === 'object' ? { ...seller.metadata } : {};
  const hash = String(meta.passwordHash || meta.password_hash || meta.senhaHash || meta.senha_hash || '').trim();
  const plain = String(meta.password || meta.senha || meta.pass || '').trim();

  if (hash) {
    try {
      if (await bcrypt.compare(password, hash)) return { ok: true, metadata: meta };
    } catch (_) {}
  }
...
app.post('/api/seller/auth/login', async (req, res) => {
  try {
    ...
    if (user) {
      ...
      if (storedHash) {
        try { valid = await bcrypt.compare(password, storedHash); } catch (_) { valid = false; }
        if (!valid && storedHash === password) {
          valid = true;
          user.passwordHash = await bcrypt.hash(password, 10);
          await user.save();
      ...
      seller = user.sellerId ? await Seller.findOne({ sellerId: user.sellerId }) : await Seller.findOne({ email });

      if (!valid && seller) {
        const metaCheck = await validateSellerMetadataPassword(seller, password);
        if (metaCheck.ok) {
          valid = true;
          seller.metadata = metaCheck.metadata;
          await seller.save();
      ...
      if (String(user.role || '').toLowerCase() !== 'seller' && !user.sellerId && !seller) {
        return res.status(403).json({ ok: false, error: 'Esta conta não está liberada como seller.' });
      }

      user.role = 'seller';
      if (seller && !user.sellerId) user.sellerId = seller.sellerId;
      await user.save();
      return res.json(await buildSellerSessionResponse(user, seller));
    }
    ...
    const metaCheck = await validateSellerMetadataPassword(seller, password);
    if (!metaCheck.ok) {
      return res.status(401).json({ ok: false, error: 'Credenciais inválidas. Se sua solicitação foi criada sem senha, crie um novo cadastro de seller ou redefina a senha.' });
    }

    user = await User.create({
      name: seller.displayName || seller.storeName || email.split('@')[0],
      email,
    ...
    console.error('Erro em /api/seller/auth/login:', error);
    return res.status(500).json({ ok: false, error: 'Erro ao fazer login do seller' });
  }
});

app.get('/api/auth/me', authRequired, (req, res) => res.json({ ok: true, user: toJSON(req.user) }));

app.get('/api/seller/auth/me', authRequired, async (req, res) => {
  try {
    const seller = req.user?.sellerId ? await Seller.findOne({ sellerId: req.user.sellerId }) : await Seller.findOne({ email: req.user.email });
    if (String(req.user?.role || '').toLowerCase() !== 'seller' && !seller) {
      return res.status(403).json({ ok: false, error: 'Esta conta não está liberada como seller.' });
    }
    return res.json(await buildSellerSessionResponse(req.user, seller));
  } catch (error) {
    return res.status(500).json({ ok: false, error: 'Erro ao validar sessão do seller' });
  }
});
