...
function financeiroErpReference(row = {}) {
  ...
  async function persistFinanceCustomerPhone({
    telefone = '',
    clienteCpf = '',
    clienteNome = '',
    clienteId = '',
    ...
    const normalizedPhone = normalizePhone(telefone, '55');
    if (!normalizedPhone) {
      const error = new Error('Informe um telefone/WhatsApp válido.');
      error.statusCode = 400;
      throw error;
    }

    const cpf = cleanPhone(clienteCpf || '');
    const nome = String(clienteNome || '').trim();
    const Person = mongoose.models.ErpPerson;
    let person = null;

    if (Person) {
      if (cpf) person = await Person.findOne({ document: cpf });
      if (!person && nome) {
...
, 'i') }
      ...
      if (person) {
        person.phone = cleanPhone(normalizedPhone);
        person.active = true;
        person.metadata = {
          ...(person.metadata || {}),
          phoneUpdatedBy: actor.email || actor.name || 'Ariana Financeiro',
          phoneUpdatedIn: 'ariana_financeiro',
          phoneUpdatedAt: new Date()
        };
        await person.save();

        if (person.linkedUserId && User) {
          await User.updateOne(
            { _id: person.linkedUserId },
            { $set: { phone: cleanPhone(normalizedPhone) } }
          ).catch(() => null);
        }
      } else if (nome) {
        person = await Person.create({
          ...
          personType: cpf.length === 14 ? 'Pessoa Jurídica' : 'Pessoa Física',
          roles: ['Cliente'],
          active: true,
          source: 'manual',
          sourceId: 'financeiro_' + new mongoose.Types.ObjectId(),
          metadata: {
            createdBy: actor.email || actor.name || 'Ariana Financeiro',
            createdIn: 'ariana_financeiro',
            createdFromPaymentPhonePrompt: true
          }
        });
      }
    }
    ...
    if (CrediarioCliente) {
      if (clienteId && mongoose.Types.ObjectId.isValid(clienteId)) {
        clienteLocal = await CrediarioCliente.findById(clienteId);
      }
      if (!clienteLocal && cpf) clienteLocal = await CrediarioCliente.findOne({ cpf });
      if (!clienteLocal && nome) {
        clienteLocal = await CrediarioCliente.findOne({
          nome: new RegExp('^' + escapeRegex(nome) + '
    try {
      const referenceRaw = String(req.params.codigo || '').trim();
      const reference = parseFinanceiroErpReference(referenceRaw);
      const body = req.body || {};
...
, 'i')
      ...
      if (clienteLocal) {
        if (nome) clienteLocal.nome = nome;
        if (cpf) clienteLocal.cpf = cpf;
        clienteLocal.telefone = normalizedPhone;
        clienteLocal.ativo = true;
        await clienteLocal.save();
      } else if (nome) {
        clienteLocal = await CrediarioCliente.create({
          nome,
          cpf,
    ...
    if (reference?.type === 'order') {
      await Order.updateOne(
        { _id: reference.orderId, origin: 'erp_ariana' },
        { $set: { customerPhone: cleanPhone(normalizedPhone) } }
      ).catch(() => null);
    } else if (reference?.type === 'ledger') {
      const Entry = mongoose.models.ErpFinancialEntry;
      if (Entry) {
        await Entry.updateOne(
          { _id: reference.entryId },
          { $set: { personPhone: cleanPhone(normalizedPhone) } }
        ).catch(() => null);
      }
    }

    if (recibo) {
      recibo.telefone = normalizedPhone;
      if (clienteLocal?._id) recibo.clienteId = clienteLocal._id;
      await recibo.save();
    }

    return {
      telefone: normalizedPhone,
      personId: person?._id ? String(person._id) : '',
      clienteId: clienteLocal?._id ? String(clienteLocal._id) : ''
    };
  }
