...
export default function createErpSigeFiscalHistoryRoutes(context={}){
  ...
  router.get('/erp/fiscal/historico/nfe/:id/danfe.pdf',context.adminRequired,async(req,res)=>{try{const doc=await service.detail(req.params.id);const pdf=danfe.generate(doc);const number=safeFilePart(doc?.number||'sem-numero');const key=safeFilePart(doc?.key||doc?.sourceId||'sem-chave');const filename=`DANFE-${number}-${key}.pdf`;res.setHeader('Content-Type','application/pdf');res.setHeader('Content-Disposition',`inline; filename="${filename}"`);res.setHeader('Content-Length',String(pdf.length));res.setHeader('Cache-Control','private, no-store');return res.status(200).send(pdf)}catch(e){return res.status(Number(e?.statusCode||500)).json({ok:false,error:e?.message||'Erro ao gerar DANFE da NF-e histórica.',code:e?.code||'ERP_DANFE_ERROR'})}});
