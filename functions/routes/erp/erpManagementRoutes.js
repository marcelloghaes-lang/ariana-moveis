...
export default function createErpManagementRoutes(context={}){
 ...
 router.get('/erp/financeiro/pagamentos',context.adminRequired,handle(async req=>({payments:await ledger.listPayments(req.query||{})})));
 router.post('/erp/financeiro/contas-receber',context.adminRequired,handle(async req=>{await settings.assertFinanceCreate(req.body||{});return{entry:await ledger.createReceivable(req.body||{},actor(req))}},201));
 router.post('/erp/financeiro/contas-pagar',context.adminRequired,handle(async req=>{await settings.assertFinanceCreate(req.body||{});return{entry:await ledger.createPayable(req.body||{},actor(req))}},201));
 ...
 router.post('/erp/financeiro/lancamentos/:id/quitar',context.adminRequired,handle(async req=>{await settings.assertFinanceMutation(req.params.id,{...(req.body||{}),__operation:'pay'});return{entry:await ledger.pay(req.params.id,req.body||{},actor(req))}}));
 ...
 router.get('/erp/relatorios/financeiro',context.adminRequired,handle(async req=>({report:await reports.financial(req.query||{})})));
