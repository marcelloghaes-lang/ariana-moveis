...
export default function createTelevendasRouter(context = {}) {
  ...
  router.post('/erp/orders/:orderId/estornar', context.adminRequired, erpHandler(async (req) => {
    const actor = erpActor(req);
    const order = await erp.estornar(req.params.orderId, req.body || {}, actor);
    await erpCash.reverseSale(order, actor);
    return { order };
  }));
