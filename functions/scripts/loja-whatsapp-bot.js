...
async function consultGustavoSupportAdvisor(
  ...
  const prompt = [
    ...
    'Se recentPaymentProof=true, preserve o contexto financeiro: interprete mensagens e áudios seguintes primeiro como continuação sobre comprovante, parcela, prestação, compra já realizada ou conferência do pagamento, salvo quando houver intenção clara de iniciar outro assunto.',
    'Nesse contexto financeiro, citar geladeira, TV, fogão, sofá ou outro produto pode identificar a compra/parcela paga e não significa automaticamente intenção de comprar; não abra catálogo apenas pelo nome do produto.',
    'Quando o cliente trouxer uma dúvida financeira que depende de conferência real ausente no contexto, responda de forma natural dizendo que será conferida/encaminhada, sem afirmar baixa, valor, parcela ou resultado não confirmado.',
    'Faça uma pergunta curta somente quando ela for necessária para obter um dado que o sistema realmente pode consultar, como identificação segura do titular; evite interrogatório e respostas robóticas.',
