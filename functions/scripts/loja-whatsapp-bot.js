...
function isGreeting(text) {
  ...
  const hello = '(?:oi+|oie+|ola+)';
  const daypart = '(?:bom dia|boa tarde|boa noite)';
  const wellbeing = '(?:tudo bem|td bem|como vai)';

  if (new RegExp(`^(?:${hello}|${daypart}|${wellbeing})import http from 'http';
import fs from 'fs';
...
function detectCategory(text) {
  ...
  return new RegExp(
    `^(?=.*(?:${hello}|${daypart}|${wellbeing}))(?:${hello})?\\s*(?:${daypart})?\\s*(?:${wellbeing})?\\s*import http from 'http';
import fs from 'fs';
