...
function getModel(name, schemaDef) {
  // Importante: não cria modelos na carga inicial do módulo.
  // Assim evitamos quebrar o server.js atual, que já declara estes modelos.
...
function ManufacturerIntegration() {
  return getModel('ManufacturerIntegration', {
...
function ManufacturerDispatchQueue() {
  return getModel('ManufacturerDispatchQueue', {
...
function IntegrationAuditLog() {
  return getModel('IntegrationAuditLog', {
...
export async function dispatchQueueItem(queueId) {
  ...
  try {
    ...
    item.attempts += 1;
    item.lastAttemptAt = new Date();
    item.lastError = error.response?.data ? JSON.stringify(error.response.data).slice(0, 1000) : String(error.message || error);
    await item.save();
    await IntegrationAuditLog().create({ eventType: 'manufacturer_order_error', manufacturer: item.manufacturer, orderId: item.orderId, queueId, status: 'error', statusCode: error.response?.status, response: error.response?.data, message: item.lastError });
    throw error;
  }
}
...
export async function registerWebhookEvent({ manufacturer, eventType, payload }) {
  return IntegrationAuditLog().create({
    ...
    status: 'received',
    response: payload || {},
    metadata: { source: 'enterprise_webhook' }
  });
}
