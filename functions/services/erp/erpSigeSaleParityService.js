...
export function createErpSigeSaleParityService(context={}){
 ...
 async function updateReceivable(orderId,number,payload={},actor={}){
  ...
  if(!['pedido','venda','faturado'].includes(String(order.status||'').toLowerCase()))throw fail('O financeiro só pode ser editado em pedido ou venda ativa.',409,'ORDER_NOT_FINANCIAL');
  const erp=order.televendas?.erp||{},rows=array(erp.receivables).map(r=>({...r,payments:array(r.payments).map(p=>({...p}))})),n=Math.max(1,Number(number||0)),idx=rows.findIndex(r=>Number(r.number)===n);
  if(idx<0)throw fail('Parcela não encontrada.',404,'RECEIVABLE_NOT_FOUND');
  const current=rows[idx];
  if(['cancelado','estornado'].includes(current.status))throw fail('Parcela cancelada/estornada não pode ser alterada.',409,'RECEIVABLE_CLOSED');

  let paid=null;
  if(payload.paid!==undefined)paid=Boolean(payload.paid);
  else if(payload.status==='recebido'||payload.status==='paid')paid=true;
  else if(payload.status==='pendente'||payload.status==='pending')paid=false;

  const hasPayments=array(current.payments).length>0||Number(current.receivedAmount||0)>0.009;
  const wasPaid=current.status==='recebido';

  if(paid===false&&hasPayments){
    throw fail('Esta parcela possui recebimentos registrados. Use o fluxo de estorno/cancelamento para preservar o histórico financeiro.',409,'RECEIVABLE_PAYMENT_IMMUTABLE');
  }

  if(payload.receivedAmount!==undefined&&paid!==true){
    const requested=money(payload.receivedAmount),existing=money(current.receivedAmount||0);
    if(Math.abs(requested-existing)>0.009){
      throw fail('Valor recebido não pode ser alterado diretamente. Use Registrar recebimento.',409,'USE_RECEIVE_ENDPOINT');
    }
  }
  ...
  if(paid===true&&!wasPaid){
    rows[idx]=next;
    const preparedErp=recalc(order,erp,rows);
    preparedErp.timeline=[...array(erp.timeline),{status:'financeiro',label:`Parcela ${n}/${next.installments||rows.length}: dados revisados antes do recebimento`,at:new Date(),by:actorName(actor)}];
    order.televendas={...(order.televendas||{}),erp:preparedErp};
    await order.save();
    await audit('erp.receivable.prepared_for_receive',order,{message:`Parcela ${n} preparada para recebimento`,number:n,dueAt:next.dueAt,method:next.method});
    return finance.receive(String(order._id),n,{
      settle:true,
      method:payload.receivedMethod||payload.method||payload.paymentMethod||next.method||order.payment?.method||'',
      paidAt:payload.receivedAt||new Date(),
      bankAccountName:payload.bankAccountName||next.bankAccountName||'',
      note:payload.notes??payload.note??next.notes??'',
      document:payload.document||payload.documento||''
    },actor);
  }

  if(paid===true&&wasPaid&&payload.receivedAmount!==undefined){
    const requested=money(payload.receivedAmount),existing=money(current.receivedAmount||0);
    if(Math.abs(requested-existing)>0.009){
      throw fail('O valor de uma parcela já recebida não pode ser reescrito. Preserve o pagamento e use estorno/correção financeira quando necessário.',409,'RECEIVABLE_PAYMENT_IMMUTABLE');
    }
  }
  ...
  rows[idx]=next;
  const nextErp=recalc(order,erp,rows);
  nextErp.timeline=[...array(erp.timeline),{status:'financeiro',label:`Parcela ${n}/${next.installments||rows.length} atualizada`,at:new Date(),by:actorName(actor)}];
  order.televendas={...(order.televendas||{}),erp:nextErp};
  await order.save();
