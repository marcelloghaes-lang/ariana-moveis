...
export function createErpSigeSaleParityService(context={}){
 ...
 async function updateReceivable(orderId,number,payload={},actor={}){
  const order=await findOrder(orderId);if(order.status!=='faturado')throw fail('O financeiro da venda só pode ser editado depois do faturamento.',409,'ORDER_NOT_BILLED');
