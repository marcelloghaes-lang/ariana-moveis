...
const schema=new mongoose.Schema({
  ...
  fiscalSerie:{type:Number,default:1},fiscalNextNumber:{type:Number,default:1},fiscalNextNumberHomologacao:Number,fiscalNextNumberProducao:Number,fiscalNaturezaOperacao:{type:String,default:'Venda de mercadoria'},
...
function numberingState(x){const environment=x.emissionEnvironment==='producao'?'producao':'homologacao',production=Math.max(1,Number(x.fiscalNextNumberProducao??x.fiscalNextNumber??1)),homologation=Math.max(1,Number(x.fiscalNextNumberHomologacao??1));return{environment,production,homologation,nextNumber:environment==='producao'?production:homologation}}
function emissionPublic(x){
  const a1Configured=Boolean(x.a1PfxEnc&&x.a1PasswordEnc),numbering=numberingState(x);return{
    type:'sefaz_a1',environment:numbering.environment,encryptionReady:Boolean(encryptionSecret()),serie:Number(x.fiscalSerie||1),nextNumber:numbering.nextNumber,nextNumberHomologacao:numbering.homologation,nextNumberProducao:numbering.production,
    ...
    configured:a1Configured&&Boolean(digits(x.fiscalCnpj)&&clean(x.fiscalIe)&&Number(x.fiscalCrt||0)&&Number(x.fiscalSerie||0)&&Number(numbering.nextNumber||0)),updatedAt:x.emissionUpdatedAt||null,updatedBy:clean(x.emissionUpdatedBy,180)
  }
}
...
export function createErpFiscalSettingsService(context={}){
  ...
  async function setFiscalConfig(payload={},actor={}){
    const x=await row(),who=clean(actor.name||actor.nome||actor.email||'Operador',180),previousEnvironment=x.emissionEnvironment==='producao'?'producao':'homologacao',environment=String((payload.environment??x.emissionEnvironment)||'homologacao').toLowerCase()==='producao'?'producao':'homologacao',state=numberingState(x),hasHom=Number.isInteger(Number(x.fiscalNextNumberHomologacao))&&Number(x.fiscalNextNumberHomologacao)>0,hasProd=Number.isInteger(Number(x.fiscalNextNumberProducao))&&Number(x.fiscalNextNumberProducao)>0;
    const serie=Number(payload.serie??x.fiscalSerie??1),targetCurrent=environment==='producao'?state.production:state.homologation,requestedNumber=Number(payload.nextNumber??targetCurrent),legacyNumber=Math.max(1,Number(x.fiscalNextNumber||1)),firstMigrationInHomologation=environment==='homologacao'&&!hasHom&&!hasProd&&requestedNumber===legacyNumber,nextNumber=environment!==previousEnvironment?targetCurrent:(firstMigrationInHomologation?1:requestedNumber),crt=Number(payload.crt??x.fiscalCrt??0);
  ...
  async function reserveNfeNumber(actor={}){const who=clean(actor.name||actor.nome||actor.email||'Operador',180),x=await row(),state=numberingState(x),environment=state.environment,field=environment==='producao'?'fiscalNextNumberProducao':'fiscalNextNumberHomologacao',initial=environment==='producao'?state.production:state.homologation;await Settings.updateOne({key:'default',[field]:{$exists:false}},{$set:{[field]:initial}});const before=await Settings.findOneAndUpdate({key:'default'},{$inc:{[field]:1},$set:{emissionUpdatedAt:new Date(),emissionUpdatedBy:who}},{new:false});const reserved=Math.max(1,Number(before?.[field]??initial));if(environment==='producao')await Settings.updateOne({key:'default'},{$set:{fiscalNextNumber:reserved+1}});await audit('erp.fiscal.nfe.number.reserved',{message:`Número de NF-e ${reserved} reservado em ${environment}`,by:who,number:reserved,environment});return reserved}
