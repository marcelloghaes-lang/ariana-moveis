...
function withCharge(row,todayStart){const lc=calculateLateCharge({principal:row.outstanding,dueAt:row.dueAt,asOf:todayStart,direction:'receivable'});return{...row,daysLate:lc.daysLate,fine:lc.fine,interest:lc.interest,charges:lc.charges,updatedOutstanding:lc.updatedTotal}}
function normalizeEntry(r,todayStart){const left=outstanding(r);return withCharge({id:`entry:${String(r._id||r.id||'')}`,source:r.origin==='sige_import'?'SIGE':'Ariana',name:safeName(r.personName),document:clean(r.personDocument,40),phone:clean(r.phone||r.personPhone,40),email:clean(r.email||r.personEmail,160),reference:clean(r.documentNumber||r.description||r.sourceId||String(r._id||''),180),installment:clean(r.installmentNumber||r.parcelNumber||'',40),dueAt:r.dueAt,value:money(r.value),paid:principalPaid(r),outstanding:left,paymentMethod:clean(r.paymentMethod,80),origin:clean(r.origin,80),orderId:clean(r.orderId,120)},todayStart)}
function normalizeOrder(order,r,todayStart){const left=outstanding(r),erp=order.televendas?.erp||{};return withCharge({id:`order:${String(order._id)}:${Number(r.number||1)}`,source:'Ariana',name:safeName(order.customerName),document:clean(order.customerCpf,40),phone:clean(order.customerPhone,40),email:clean(order.customerEmail,160),reference:clean(erp.code||String(order._id).slice(-8).toUpperCase(),180),installment:`${Number(r.number||1)}/${Number(r.installments||1)}`,dueAt:r.dueAt,value:money(r.value),paid:principalPaid(r),outstanding:left,paymentMethod:clean(r.method||order.payment?.method,80),origin:'ariana_sale',orderId:String(order._id)},todayStart)}
...
function pdfFromReport(report){
 ...
 p.text(M,y,`Clientes: ${report.summary.clients}`,8.5,true);p.text(M+105,y,`Parcelas: ${report.summary.entries}`,8.5,true);p.text(M+205,y,`Principal: ${brl(report.summary.totalPrincipal)}`,8.5,true);p.text(M+340,y,`Encargos: ${brl(report.summary.totalCharges)}`,8.5,true);p.text(M+440,y,`Atualizado: ${brl(report.summary.totalUpdated)}`,9,true,CONTENT_W-440,'right');y+=18;
 p.text(M,y,'Regra: multa única de 2% + juros simples de 1% ao mês, proporcionais aos dias de atraso.',7);y+=13;
 for(const g of report.clients){need(48);p.rect(M,y,CONTENT_W,30,.7);p.text(M+6,y+10,g.name,9.5,true,210);p.text(M+220,y+10,formatDoc(g.document),7.5,false,105);p.text(M+330,y+10,`${g.count} parcela(s)`,7.5,true,65,'center');p.text(M+400,y+8,`Principal ${brl(g.totalPrincipal)}`,6.7,false,125,'right');p.text(M+400,y+18,`Atualizado ${brl(g.totalUpdated)}`,7.4,true,125,'right');y+=36;
   const headers=[['Venc.',50],['Referência',105],['Parc.',38],['Dias',30],['Principal',72],['Multa',60],['Juros',60],['Atualizado',85]];let x=M;p.line(M,y,M+CONTENT_W,y,.4);for(const [h,w] of headers){p.text(x+2,y+10,h,6.4,true,w-4,h==='Principal'||h==='Multa'||h==='Juros'||h==='Atualizado'?'right':'left');x+=w}y+=14;
   for(const r of g.entries){need(18);x=M;const vals=[[formatDate(r.dueAt),50],[r.reference,105],[r.installment||'-',38],[String(r.daysLate),30],[brl(r.outstanding),72],[brl(r.fine),60],[brl(r.interest),60],[brl(r.updatedOutstanding),85]];for(let i=0;i<vals.length;i++){const [v,w]=vals[i];p.text(x+2,y+10,v,6.3,false,w-4,i>=4?'right':'left');x+=w}p.line(M,y+14,M+CONTENT_W,y+14,.2);y+=15}
   y+=8;
 }
 if(p)pushPage();return buildPdf(pages)
}
...
export function createErpDelinquencyReportService(context={}){
 ...
 async function rawRows(q={}){
   ...
   const rows=legacy.filter(r=>!(r.origin==='sige_import'&&Math.abs(Number(r.value||0))>=10000000)).map(r=>normalizeEntry(r,pr.todayStart)).filter(r=>r.outstanding>0.009);
   const orders=await Order.find({origin:'erp_ariana',status:'faturado','televendas.erp.receivables.0':{$exists:true}}).select('_id customerName customerCpf customerPhone customerEmail payment televendas updatedAt').lean();
 ...
 async function report(q={}){
  const {period:pr,rows}=await rawRows(q),map=new Map();
  for(const r of rows){const k=clientKey(r),g=map.get(k)||{key:k,name:r.name,document:r.document,phone:r.phone,email:r.email,count:0,totalPrincipal:0,totalFine:0,totalInterest:0,totalCharges:0,totalUpdated:0,oldestDue:r.dueAt,newestDue:r.dueAt,entries:[]};g.count++;g.totalPrincipal+=Number(r.outstanding||0);g.totalFine+=Number(r.fine||0);g.totalInterest+=Number(r.interest||0);g.totalCharges+=Number(r.charges||0);g.totalUpdated+=Number(r.updatedOutstanding||r.outstanding||0);if(new Date(r.dueAt)<new Date(g.oldestDue))g.oldestDue=r.dueAt;if(new Date(r.dueAt)>new Date(g.newestDue))g.newestDue=r.dueAt;if(!g.phone&&r.phone)g.phone=r.phone;if(!g.email&&r.email)g.email=r.email;g.entries.push(r);map.set(k,g)}
  const clients=[...map.values()].map(g=>({...g,totalPrincipal:money(g.totalPrincipal),totalFine:money(g.totalFine),totalInterest:money(g.totalInterest),totalCharges:money(g.totalCharges),totalUpdated:money(g.totalUpdated),totalOverdue:money(g.totalUpdated),entries:g.entries.sort((a,b)=>new Date(a.dueAt)-new Date(b.dueAt))})).sort((a,b)=>compareNames(a.name,b.name));
  return{period:{from:pr.from,to:pr.to,fromIso:pr.fromIso,toIso:pr.toIso,today:pr.todayIso},policy:ERP_LATE_CHARGE_POLICY,summary:{clients:clients.length,entries:rows.length,totalPrincipal:sum(rows,r=>r.outstanding),totalFine:sum(rows,r=>r.fine),totalInterest:sum(rows,r=>r.interest),totalCharges:sum(rows,r=>r.charges),totalUpdated:sum(rows,r=>r.updatedOutstanding),totalOverdue:sum(rows,r=>r.updatedOutstanding),oldestDue:rows.length?rows.reduce((a,r)=>!a||new Date(r.dueAt)<new Date(a)?r.dueAt:a,null):null},clients}
 }
