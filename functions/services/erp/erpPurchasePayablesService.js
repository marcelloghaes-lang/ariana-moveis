...
export function createErpPurchasePayablesService(context={}){
  ...
  async function generate(id,payload={},actor={}){
    ...
    if(existing.length){const wanted=new Set(plan.installments.map(i=>i.documentNumber)),existingDocs=new Set(existing.map(e=>String(e.documentNumber||'')));const incompatible=existing.length>plan.installments.length||[...existingDocs].some(d=>!wanted.has(d));if(incompatible)throw fail('Esta compra já possui um plano de parcelas diferente no Contas a Pagar. Para evitar duplicidade, ajuste os lançamentos existentes no Financeiro antes de gerar outro plano.',409,'PURCHASE_PAYABLES_ALREADY_GENERATED')}
