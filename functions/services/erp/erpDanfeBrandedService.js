...
class Canvas{
  ...
  rect(x,y,w,h,lw=.5){this.ops.push(`0 G ${lw} w ${x.toFixed(2)} ${(PAGE_H-y-h).toFixed(2)} ${w.toFixed(2)} ${h.toFixed(2)} re S`)}
  ...
  text(x,y,s,size=6,bold=false,align='left',maxW=null){
    ...
    this.ops.push(`BT /${bold?'F2':'F1'} ${size.toFixed(2)} Tf 1 0 0 1 ${Math.max(0,xx).toFixed(2)} ${(PAGE_H-y-size).toFixed(2)} Tm (${escPdf(s)}) Tj ET`);
  }
...
function header(c,d,y,hasMascot){
  ...
  y+=stubH+7;
  c.line(M,y-3,M+W,y-3,.35,[2,2]);

  const hh=86,left=282,right=W-left;
  c.rect(M,y,left,hh);
  c.rect(M+left,y,right,hh);
  if(hasMascot)c.image(M+12,y+10,50,52);
  const brandX=M+72;
  const arianaSize=13;
  c.text(brandX,y+8,'ARIANA',arianaSize,true);
  const moveisX=brandX+c.width('ARIANA',arianaSize,true)+22;
  c.text(moveisX,y+10,'MÓVEIS',9.2,true);
  c.text(brandX,y+24,'Sua casa merece o melhor',5.5,true,'left',left-80);
  const ns=c.fit(d.issuer.name,5.4,left-80,4.4,true);
  c.text(brandX,y+35,d.issuer.name,ns,true,'left',left-80);
  c.wrap(`${d.issuer.street} - ${d.issuer.city}/${d.issuer.uf} - CEP ${fmtCep(d.issuer.cep)}`,4.8,left-80,2).forEach((ln,i)=>c.text(brandX,y+46+i*7,ln,4.8,false,'left',left-80));
  c.text(brandX,y+64,`CNPJ/CPF: ${fmtDoc(d.issuer.doc)}   IE: ${d.issuer.ie||''}`,4.6,false,'left',left-80);
  c.text(brandX,y+73,`Fone: ${fmtPhone(d.issuer.phone)}`,4.6,false,'left',left-80);
  ...
  c.text(rx+8,y+31,'0 - Entrada',5,false,'left',52);
  c.text(rx+8,y+39,'1 - Saída',5,false,'left',52);
  c.rect(rx+64,y+29,17,18,.5);
  c.text(rx+64,y+33,d.tpNF==='0'?'0':'1',8,true,'center',17);
  c.text(rx+92,y+30,`Nº ${d.number}`,7,true,'left',72);
  c.text(rx+92,y+39,`SÉRIE ${d.series}`,6,true,'left',72);
  c.text(rx+92,y+48,'FOLHA 1/1',5,false,'left',72);
  const statusX=rx+168,statusW=right-176;
  c.rect(statusX,y+29,statusW,24,.45);
  c.text(statusX+2,y+31,'SITUAÇÃO FISCAL',3.8,false,'center',statusW-4);
  const status=fiscalStatus(d.status);
  const statusSize=c.fit(status,5.8,statusW-4,3.9,true);
  c.text(statusX+2,y+40,status,statusSize,true,'center',statusW-4);
  c.barcode(rx+8,y+57,right-16,16,d.key);
...
function draw(doc,mascotJpeg){
  const d=parseDanfe(doc),hasMascot=Boolean(jpegSize(mascotJpeg)),c=new Canvas();
  let y=M;const W=PAGE_W-M*2;
  y=header(c,d,y,hasMascot);
  ...
  for(let r=0;r<maxRows;r++){
    const it=d.items[r];x=M;c.rect(M,y,W,rowH);
  ...
  if(d.items.length>maxRows){c.rect(M,y,W,13);c.text(M+3,y+4,`+ ${d.items.length-maxRows} item(ns) adicional(is). Consulte o XML autorizado para a relação completa.`,4.2,true,'left',W-6);y+=13}
  if(y<footerStart)y=footerStart;
  ...
  return buildPdf(c.stream(),hasMascot?mascotJpeg:null);
}
