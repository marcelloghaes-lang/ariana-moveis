...
export function createErpParityAnalyticsService(context={}){
 ...
 async function finance(q={}){
  ...
  const dm=new Map();
  for(const r of recOver){const k=(r.personDocument||r.personName||'sem-identificacao').toLowerCase(),x=dm.get(k)||{personName:r.personName||'Sem identificação',personDocument:r.personDocument||'',count:0,total:0,oldestDue:r.dueAt};x.count++;x.total+=outstanding(r);if(new Date(r.dueAt)<new Date(x.oldestDue))x.oldestDue=r.dueAt;dm.set(k,x)}
 ...
 async function sales(q={}){
  ...
  const customers=new Map(),products=new Map(),sellers=new Map(),payments=new Map();let total=0,itemCount=0;
  for(const s of rows){total+=s.total;const ck=(s.customerName||'Consumidor').trim().toLowerCase(),c=customers.get(ck)||{name:s.customerName||'Consumidor',orders:0,value:0};c.orders++;c.value+=s.total;customers.set(ck,c);const sk=(s.sellerName||'Sem vendedor').trim().toLowerCase(),sv=sellers.get(sk)||{name:s.sellerName||'Sem vendedor',orders:0,value:0};sv.orders++;sv.value+=s.total;sellers.set(sk,sv);const pk=s.paymentMethod||'Não informado';payments.set(pk,(payments.get(pk)||0)+s.total);for(const i of s.items){itemCount+=i.qty;const key=i.productId||String(i.name||'produto').toLowerCase(),p=products.get(key)||{name:i.name||'Produto',qty:0,value:0};p.qty+=i.qty;p.value+=i.value;products.set(key,p)}}
  const ranking=[...customers.values()].sort((a,b)=>b.value-a.value).map(x=>({...x,value:money(x.value)}));let acc=0;const abc=ranking.map(x=>{acc+=x.value;const cumulative=total?acc/total:0;return{...x,share:total?money(x.value/total*100):0,curve:cumulative<=.8?'A':cumulative<=.95?'B':'C'}});
  return{period:{from,to},summary:{sales:rows.length,items:itemCount,total:money(total),averageTicket:rows.length?money(total/rows.length):0,historical:rows.filter(x=>x.source==='historico').length,current:rows.filter(x=>x.source==='ariana').length},customerRanking:ranking.slice(0,500),customerABC:abc.slice(0,1000),productRanking:[...products.values()].sort((a,b)=>b.value-a.value).slice(0,500).map(x=>({...x,value:money(x.value)})),sellerRanking:[...sellers.values()].sort((a,b)=>b.value-a.value).slice(0,200).map(x=>({...x,value:money(x.value)})),paymentMethods:Object.fromEntries([...payments].map(([k,v])=>[k,money(v)])),sales:rows.slice(0,2000).map(x=>({...x,total:money(x.total)}))}
 }
