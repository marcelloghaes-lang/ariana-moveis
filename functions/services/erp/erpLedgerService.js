...
const entrySchema=new mongoose.Schema({
 ...
 notes:String,origin:{type:String,default:'manual',index:true},orderId:String,agreementId:{type:String,index:true},agreementNumber:String,renegotiatedAt:{type:Date,index:true},renegotiatedBy:String,installmentNumber:Number,installments:Number,createdBy:String,updatedBy:String,paidBy:String,cancelledAt:Date,cancelledBy:String
},base);
...
export function createErpLedgerService(context={}){
 ...
 async function updateEntry(id,payload={},actor={}){
  const row=await Entry.findById(id);if(!row)throw fail('Lançamento não encontrado.',404);if(row.status==='cancelled')throw fail('Lançamento cancelado não pode ser editado.',409);if(row.renegotiatedAt)throw fail('Lançamento original de acordo não pode ser alterado. Edite o acordo vigente.',409,'ENTRY_RENEGOTIATED');
 ...
 async function pay(id,payload={},actor={}){
  const row=await Entry.findById(id);if(!row)throw fail('Lançamento não encontrado.',404);if(row.status==='cancelled')throw fail('Lançamento cancelado não pode ser quitado.',409);if(row.renegotiatedAt)throw fail('Esta dívida foi substituída por um acordo. Receba pelas parcelas do acordo.',409,'ENTRY_RENEGOTIATED');
 ...
 async function cancel(id,actor={}){const row=await Entry.findById(id);if(!row)throw fail('Lançamento não encontrado.',404);if(row.renegotiatedAt)throw fail('A dívida original está vinculada a um acordo e não pode ser cancelada isoladamente.',409,'ENTRY_RENEGOTIATED');if(principalPaid(row)>0)throw fail('Desfaça os pagamentos antes de cancelar.',409);row.status='cancelled';row.cancelledAt=new Date();row.cancelledBy=actorName(actor);await row.save();await audit('erp.ledger.entry.cancelled',{message:'Lançamento cancelado',entryId:String(row._id),by:actorName(actor)});return decorate(row)}
 async function listPayments(query={}){
  const filter={status:{$ne:'cancelled'},renegotiatedAt:{$exists:false}};if(['receivable','payable'].includes(query.direction))filter.direction=query.direction;if(query.bankAccountId)filter.bankAccountId=clean(query.bankAccountId,120);const rows=await Entry.find(filter).sort({updatedAt:-1}).limit(5000).lean(),out=[];const q=clean(query.q||query.search,160).toLowerCase();
