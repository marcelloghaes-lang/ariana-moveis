...
function clientHeader(w,c){w.need(36);w.p.rect(M,w.y,CW,27,.6);w.p.text(M+6,w.y+10,c.name||c.clientName||'Sem identificação',9,true,270);w.p.text(M+280,w.y+10,c.document||c.clientDocument||'',7,false,115);w.p.text(M+400,w.y+10,money(c.totalOverdue??c.openBalance??0),8.5,true,CW-406,'right');w.y+=33}
...
export function createErpCollectionPdfService(context={}){
  ...
  async function queuePdf(q={}){
    ...
    w.box('Clientes na fila',String(data.summary.clients));w.box('Parcelas/títulos',String(data.summary.installments));w.box('Saldo vencido',money(data.summary.totalOverdue));w.box('Promessas para hoje',String(data.summary.promisesToday));w.box('Promessas atrasadas',String(data.summary.promisesLate));
    w.text('A fila agrupa uma cobrança por cliente; as parcelas permanecem detalhadas abaixo.',8,false);w.text('Multa e juros não são inventados pelo relatório: somente encargos oficialmente registrados no financeiro compõem movimentações.',7,false);
    for(const c of data.clients){clientHeader(w,c);w.row(['Vencimento','Referência','Parcela','Dias','Saldo','Tratativa'],[65,145,52,42,93,134],true);for(const r of c.entries){const st=r.case?.status||'sem tratativa';w.row([date(r.dueAt),r.reference||'—',r.installment||'—',String(r.daysLate||0),money(r.outstanding),st],[65,145,52,42,93,134])}w.y+=7}
  ...
  async function recoveryPdf(q={}){
    const data=await workflow.recuperacao(q),w=writer('RECUPERAÇÃO DE CRÉDITO','Indicadores operacionais de cobrança');w.box('Clientes inadimplentes',String(data.summary.overdueClients));w.box('Parcelas vencidas',String(data.summary.overdueInstallments));w.box('Saldo vencido',money(data.summary.overdueAmount));w.box('Promessas ativas',String(data.summary.activePromises));w.box('Promessas hoje',String(data.summary.promisesToday));w.box('Promessas atrasadas',String(data.summary.promisesLate));w.box('Tratativas concluídas no mês',String(data.summary.concludedThisMonth));w.text('Faixas de atraso',10,true);w.row(['Faixa','Clientes','Saldo'],[250,90,191],true);for(const b of data.bands)w.row([b.label,String(b.clients),money(b.value)],[250,90,191]);return{buffer:w.finish(),filename:'recuperacao-de-credito.pdf',data};
  }
