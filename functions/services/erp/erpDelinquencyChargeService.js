...
function dayInTimeZone(value){const d=new Date(value);if(Number.isNaN(d.getTime()))return null;const parts=new Intl.DateTimeFormat('en-CA',{timeZone:TZ,year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(d),x=Object.fromEntries(parts.filter(p=>p.type!=='literal').map(p=>[p.type,p.value]));return new Date(`${x.year}-${x.month}-${x.day}T00:00:00-03:00`)}
function daysOverdue(value,today=startToday()){const due=dayInTimeZone(value);if(!due)return 0;return Math.max(0,Math.floor((today.getTime()-due.getTime())/86400000))}
function chargeValues(r={},policy={}){const principal=outstanding(r),days=daysOverdue(r.dueAt),fineStored=firstDefined(r,['fineAmount','fineDefault','fine','multa']),interestStored=firstDefined(r,['interestAmount','interestDefault','interest','juros']),correctionStored=firstDefined(r,['correctionAmount','monetaryCorrection','correction','correcao']),fine=fineStored.defined?Math.max(0,money(fineStored.value)):(days>0?money(principal*Math.max(0,Number(policy.finePercent||0))/100):0),interest=interestStored.defined?Math.max(0,money(interestStored.value)):(days>0?money(principal*Math.max(0,Number(policy.monthlyInterestPercent||0))/100*(days/30)):0),correction=Math.max(0,money(correctionStored.value));return{principal,fine,interest,correction,daysOverdue:days,updated:money(principal+fine+interest+correction),chargeSource:{fine:fineStored.defined?'stored':'calculated',interest:interestStored.defined?'stored':'calculated',correction:correctionStored.defined?'stored':'none'}}}
...
async function sendEvolution(phone,message,cfg=evolutionConfig()){if(!cfg.base||!cfg.apiKey||!cfg.instance)throw fail('WhatsApp principal da loja não está configurado no backend.',503,'ERP_COLLECTION_WHATSAPP_NOT_CONFIGURED');const response=await fetch(`${cfg.base}/message/sendText/${encodeURIComponent(cfg.instance)}`,{method:'POST',headers:{'Content-Type':'application/json',apikey:cfg.apiKey},body:JSON.stringify({number:phone,text:message,delay:0,linkPreview:false}),signal:AbortSignal.timeout(30000)});const data=await readEvolutionResponse(response);return{provider:'evolution',instance:cfg.instance,senderPhone:cfg.senderPhone,messageId:extractMessageId(data),response:data}}
...
export function createErpDelinquencyChargeService(context={}){
  ...
  async function findCurrentPerson(base={}){
    ...
    if(!person&&name){const rx=new RegExp('^'+escRx(name)+'$','i');person=await Person.findOne({$or:[{name:rx},{companyName:rx}],active:{$ne:false}}).sort({updatedAt:-1}).lean()}
    return person;
  }
  ...
  function rowSummary(raw={},extra={},policy={}){const values=chargeValues(raw,policy);return{installment:clean(extra.installment||raw.installmentNumber||raw.parcelNumber||raw.number||'',50),dueAt:raw.dueAt||null,reference:clean(extra.reference||raw.documentNumber||raw.description||raw.sourceId||'',180),...values}}

  async function financialEntryContext(entryId,policy={}){
    ...
    const items=effective.map((r,index)=>rowSummary(r,{installment:`${Number(r.installmentNumber||r.parcelNumber||index+1)}/${effective.length}`,reference},policy));
  ...
  async function orderContext(orderId,number,policy={}){if(!mongoose.isValidObjectId(orderId))throw fail('Venda do ERP inválida.',404,'DELINQUENCY_TARGET_NOT_FOUND');const order=await Order.findById(orderId).lean();if(!order||order.origin!=='erp_ariana')throw fail('Venda do Ariana ERP não encontrada.',404,'DELINQUENCY_TARGET_NOT_FOUND');const today=startToday(),erp=order.televendas?.erp||{},all=arr(erp.receivables),selected=all.find(r=>Number(r.number||1)===Number(number||1));if(!selected)throw fail('Parcela da venda não encontrada.',404,'DELINQUENCY_TARGET_NOT_FOUND');const rows=all.filter(r=>{const st=String(r.status||'').toLowerCase(),due=new Date(r.dueAt||0);return!['recebido','cancelado','estornado','paid'].includes(st)&&!Number.isNaN(due.getTime())&&due<today&&outstanding(r)>0.009});const items=rows.map(r=>rowSummary(r,{installment:`${Number(r.number||1)}/${Number(r.installments||1)}`,reference:clean(erp.code||String(order._id).slice(-8).toUpperCase(),180)},policy));const contact=await currentCustomer({name:order.customerName,document:order.customerCpf,email:order.customerEmail,phone:order.customerPhone});return{targetId:`order:${orderId}:${Number(number||1)}`,purchaseKey:`order:${orderId}`,orderId:String(order._id),reference:clean(erp.code||String(order._id).slice(-8).toUpperCase(),180),contact,items};
  }

  async function contextFor(targetId,policy={}){const id=clean(targetId,260);if(id.startsWith('entry:'))return financialEntryContext(id.slice(6),policy);if(id.startsWith('order:')){const parts=id.split(':');return orderContext(parts[1],parts[2],policy)}throw fail('Parcela/título de cobrança inválido.',404,'DELINQUENCY_TARGET_NOT_FOUND')}

  async function build(targetId,template='visita'){
    const selected=TEMPLATES.has(clean(template,30))?clean(template,30):'visita',settings=await getErpSettingsSnapshot(),policy={finePercent:Math.max(0,Number(settings?.finance?.delinquencyFinePercent??2)),monthlyInterestPercent:Math.max(0,Number(settings?.finance?.delinquencyMonthlyInterestPercent??1))},ctx=await contextFor(targetId,policy);if(!ctx.items.length)throw fail('Esta compra não possui parcelas vencidas em aberto.',409,'NO_OVERDUE_INSTALLMENTS');const totals={principal:money(ctx.items.reduce((s,x)=>s+x.principal,0)),fine:money(ctx.items.reduce((s,x)=>s+x.fine,0)),interest:money(ctx.items.reduce((s,x)=>s+x.interest,0)),correction:money(ctx.items.reduce((s,x)=>s+x.correction,0)),updated:money(ctx.items.reduce((s,x)=>s+x.updated,0))};const phoneInfo=validRecipientPhone(ctx.contact.phone);if(!phoneInfo.valid)throw fail('O cliente não possui um WhatsApp válido no cadastro atual do Ariana ERP.',409,'CUSTOMER_WHATSAPP_INVALID');const message=messageFor(selected,ctx.contact.name,totals.updated,totals);return{...ctx,template:selected,phone:phoneInfo.normalized,policy,totals,message,chargesNote:`Multa: ${policy.finePercent}% sobre o saldo vencido. Juros de mora: ${policy.monthlyInterestPercent}% ao mês, proporcionais aos dias de atraso. Correção monetária somente quando houver valor registrado no título.`};
  }

  async function preview(targetId,template='visita'){const data=await build(targetId,template);const cfg=await collectionEvolutionConfig();return{...data,sender:{phone:cfg.senderPhone,instance:cfg.instance},templates:{visita:messageFor('visita',data.contact.name,data.totals.updated,data.totals),retorno:messageFor('retorno',data.contact.name,data.totals.updated,data.totals)}}}
