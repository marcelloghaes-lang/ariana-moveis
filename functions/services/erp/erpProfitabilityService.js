...
export function createErpProfitabilityService(context={}){
  ...
  async function setCost(productId,payload={},actor={}){
    ...
    if(payload.costPrice===null||payload.costPrice==='')set={costPrice:null,costSource:'',costNotes:'',costUpdatedAt:stamp,costUpdatedBy:by};
    else{const cost=Number(payload.costPrice);if(!Number.isFinite(cost)||cost<0)throw fail('Informe um custo válido maior ou igual a zero.',400,'INVALID_COST');set={costPrice:money(cost),costSource:clean(payload.costSource||'manual',40),costNotes:clean(payload.costNotes||'',500),costUpdatedAt:stamp,costUpdatedBy:by};}
  ...
  async function report(q={}){
    ...
    const products=oids.length?await Product.collection.find({_id:{$in:oids}},{projection:{name:1,sku:1,costPrice:1,costUpdatedAt:1}}).toArray():[];
    ...
    for(const o of orders){
      ...
      for(const i of arr(o.items)){
        ...
        const key=String(i.productId||i.sku||i.name||'produto'),x=productMap.get(key)||{productId:String(i.productId||''),name:i.name||p?.name||'Produto',sku:i.sku||p?.sku||'',qty:0,revenue:0,cost:0,profit:0,knownQty:0,knownRevenue:0,missingQty:0};x.qty+=qty;x.revenue+=revenue;if(hasCost){x.cost+=cost;x.profit+=profit;x.knownQty+=qty;x.knownRevenue+=revenue}else x.missingQty+=qty;productMap.set(key,x);
      }
    ...
    const productRanking=[...productMap.values()].map(x=>({...x,revenue:money(x.revenue),knownRevenue:money(x.knownRevenue),cost:money(x.cost),profit:money(x.profit),margin:x.knownRevenue?money(x.profit/x.knownRevenue*100):null,costCoverage:x.qty?money(x.knownQty/x.qty*100):0})).sort((a,b)=>b.profit-a.profit);
