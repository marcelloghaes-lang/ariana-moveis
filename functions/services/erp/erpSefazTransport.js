...
function normalizeSoapResponse(xml=''){
  ...
  if(/<(?:retEnviNFe|retConsSitNFe|retDistDFeInt)\b/i.test(source)){
    return `<soap:Envelope xmlns:soap="${SOAP12_NS}"><soap:Body>${source}</soap:Body></soap:Envelope>`;
  }
  ...
  return{
    async send(req={}){
      ...
      if(response.status<200||response.status>=300){
        throw new Error(`SEFAZ/MG respondeu HTTP ${response.status}: ${snippet(raw)}`);
      }
