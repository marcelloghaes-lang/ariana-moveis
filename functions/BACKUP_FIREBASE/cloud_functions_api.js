...
/* MIGRAÇÃO AUTOMÁTICA DE COMPATIBILIDADE: NODE LEGADO -> NODE/MONGO
 * Objetivo: preservar o arquivo original completo e trocar a infraestrutura base
 * de funções/server legado por uma camada compatível em MongoDB.
 * Atenção: triggers legados viram handlers compatíveis e precisam ser acionados
...
function __compatGetbanco de dadosInstance() {
  if (!__compatState.firestoreInstance) __compatState.firestoreInstance = new __compatbanco de dados();
...
const __compatAdmin = {
  ...
  firestore: Object.assign(function firestore() {
    return __compatGetbanco de dadosInstance();
  }, {
...
__compatModule.prototype.require = function patchedCompatRequire(id) {
  if (id === "legacy-admin") return __compatAdmin;
  if (id === "legacy-functions/v2/https") return { onRequest: __compatOnRequest };
  if (id === "legacy-functions/v2") return { setGlobalOptions: __compatSetGlobalOptions };
  if (id === "legacy-functions/v2/firestore") return { onDocumentWritten: __compatOnDocumentWritten };
  return __compatOriginalRequire.apply(this, arguments);
};

/* eslint-disable */
const { onRequest } = { onRequest: __compatOnRequest };
const { setGlobalOptions } = { setGlobalOptions: __compatSetGlobalOptions };
const { onDocumentWritten } = { onDocumentWritten: __compatOnDocumentWritten };
const initPagarmeManufacturerModule = require("./pagarme-manufacturer-module");

setGlobalOptions({ region: "southamerica-east1" });

// Camada admin compatível (para triggers legados fora do Express)
const adminGlobal = __compatAdmin;
...
const WHATSAPP_EVOLUTION_DEFAULT_WEBHOOK_URL = __compatEnvFirst("WHATSAPP_EVOLUTION_WEBHOOK_URL", "APP_BASE_URL") ? `${__compatEnvFirst("WHATSAPP_EVOLUTION_WEBHOOK_URL", "APP_BASE_URL").replace(/\/+$/, "")}/api/whatsapp-callback` : "http://localhost:3000/api/whatsapp-callback";
...
function buildApp() {
  ...
  const RESET_CONTINUE_URL =
    ...
    (__compatEnvFirst("APP_BASE_URL") ? `${__compatEnvFirst("APP_BASE_URL").replace(/\/+$/, "")}/login_seller.html` : "http://localhost:3000/login_seller.html");
  ...
  function externalId(order) {
    return String(order?.id_legacy || order?.id || order?.orderId || order?.external_id || "").trim();
  }
  ...
  async function getManufacturerProfileFrombanco de dados(manufacturer) {
  ...
  function normalizeManufacturerOrderPayload(input = {}, existing = {}) {
    ...
    const orderId = String(
      src.orderId || src.id || src.id_legacy || src.external_id || src.idExterno || ""
    ).trim();
  ...
  async function dispatchOrderToManufacturer(orderInput = {}) {
    const firestoreProfile = await getManufacturerProfileFrombanco de dados(resolveManufacturerFromInput(orderInput));
...
function makeIdempotencyKey(req, fallbackSeed) {
  ...
  // Fluxo:
  // 1) Front chama GET /mp/oauth/url com header Authorization: Bearer <JWT do seller>
  ...
  async function requireSellerAuth(req) {
    ...
    // confirma que é seller no banco de dados
...
if (!orderId) return res.status(400).json({ ok: false, error: "orderId é obrigatório" });
      ...
      // (Opcional) salva em banco de dados já no momento da criação
      await mergeOrder(orderId, {
        status: "pending_payment",
        payment: {
          provider: "mercadopago",
  ...
  //
  // Para reduzir custo/limite, guardamos coordenadas em cache no banco de dados: coleção "_geo_cache".
...
// ================= reCAPTCHA (Enterprise) - Update Phone =================
  function getProjectId() {
    return (
      ...
      (() => {
        try {
          const cfg = JSON.parse(process.env.legacy_CONFIG || "{}");
  ...
  app.post("/manufacturers/orders/dispatch", async (req, res) => {
    try {
      const body = req.body || {};
      const orderId = String(body.orderId || body.id || body.id_legacy || "").trim();
  ...
  app.get("/manufacturers/integrations/:manufacturer", async (req, res) => {
    try {
      ...
      const firestoreProfile = await getManufacturerProfileFrombanco de dados(manufacturer);
  ...
  app.post("/manufacturers/integrations/upsert", async (req, res) => {
    try {
      ...
      const saved = await getManufacturerProfileFrombanco de dados(payload.manufacturer);
...
// ===== Bootstrap Node local sem legacy Functions =====
