...
// ============================================================
// SIGE CLOUD -> VENDA/PEDIDO PARA EMISSÃO MANUAL DE NF-e
// Fluxo seguro: Ariana cria a venda no SIGE, você emite a NF-e
// manualmente no SIGE, e depois o backend sincroniza XML/DANFE.
// Este módulo é incremental e não altera rotas antigas.
// ============================================================
...
function arianaSigeBuildVendaPayloadFromOrder(order = {}, body = {}) {
  ...
  // Se o usuário enviar payload pronto do SIGE, respeita 100%.
...
function arianaSigeBuildPlanoContaPayloadVariants(payload = {}) {
  ...
  // 2) Depois testa sem PlanoDeConta. Em algumas contas o SIGE usa o padrão do PDV/ERP
  // ou só exige esse campo quando ele é enviado com valor não reconhecido.
  pushVariant('sem-plano', (clone) => {
    delete clone.PlanoDeConta;
  });

  // 3) Mantém o payload original como última alternativa, apenas para debug/comparação.
...
async function arianaSigePesquisarPessoa({ nome = '', documento = '', email = '' } = {}) {
  ...
  for (const params of attempts) {
    try {
      ...
      // Pesquisa sem resultado pode variar por conta SIGE. O cadastro será tentado em seguida.
    }
  }
...
async function arianaSigeEnsurePessoaForOrder(order, vendaPayload = {}, body = {}) {
  ...
  if (!nome) {
    const err = new Error('Não foi possível cadastrar cliente no SIGE: nome do cliente ausente.');
    err.statusCode = 400;
    throw err;
  }
  ...
  try {
    ...
    error.message = `Não foi possível cadastrar o cliente no SIGE antes da venda: ${error.message || String(error)}`;
    throw error;
  }
}
...
app.post('/api/admin/sige/orders/:orderId/criar-venda', adminRequired, async (req, res) => {
  try {
    const order = await enterpriseCompatFindOrder(req.params.orderId);
    if (!order) return res.status(404).json({ ok: false, error: 'Pedido não encontrado para criar venda no SIGE' });
...
app.get('/api/admin/sige/orders/:orderId/venda', adminRequired, async (req, res) => {
  try {
    const order = await enterpriseCompatFindOrder(req.params.orderId);
    if (!order) return res.status(404).json({ ok: false, error: 'Pedido não encontrado para consultar venda SIGE' });
...
app.post('/api/admin/sige/orders/:orderId/sync-nfe', adminRequired, async (req, res) => {
  try {
    const order = await enterpriseCompatFindOrder(req.params.orderId);
    if (!order) return res.status(404).json({ ok: false, error: 'Pedido não encontrado para sincronizar NF-e do SIGE' });
    ...
    if (!invoiceData?.xmlUrl && !invoiceData?.danfeUrl && !invoiceData?.accessKey && !invoiceData?.number) {
      return res.status(404).json({
        ok: false,
        error: 'NF-e ainda não localizada no SIGE para esta venda/pedido',
        hint: 'Emita a NF-e manualmente no SIGE a partir da venda criada. Depois rode este sync novamente. Se necessário, envie xmlUrl/danfeUrl/accessKey no body.',
        queryUsed: query
      });
    }
...
async function consultarSigeNfeFiscal(order, query = {}) {
  ...
  if (!params.CodigoNFe) {
    const err = new Error('Informe o número da NF-e em codigo/numero ou salve a NF-e no pedido antes da consulta.');
    err.statusCode = 400;
    throw err;
  }
...
async function searchSigeInvoiceForOrder(order, query = {}) {
  // Preferência: endpoint fiscal oficial encontrado no Swagger.
...
app.post('/api/admin/sige/orders/:orderId/emitir-nfe', adminRequired, async (req, res) => {
  try {
    ...
    // Caminho manual/assistido: se já tiver XML/DANFE/chave/número no body, apenas grava no pedido.
