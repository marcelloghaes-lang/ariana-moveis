...
function buildTrackingLine(order = {}, trackingCode = '') { const code = String(trackingCode || order.trackingCode || order.tracking_code || '').trim(); return code ? ` CÃ³digo de rastreio: ${code}` : ''; }
function buildOrderStatusMessage(orderId, order = {}, settings = {}) { const template = String(settings.statusTemplate || DEFAULT_WHATSAPP_SETTINGS.statusTemplate).trim(); const replacements = { customerName: extractOrderCustomerName(order), orderId: String(orderId || order.id || order.orderId || '').trim() || '---', status: String(order.status || order.statusLabel || 'Atualizado').trim(), trackingCode: String(order.trackingCode || order.tracking_code || '').trim(), trackingLine: buildTrackingLine(order), storeName: 'Ariana MÃ³veis' }; return template.replace(/\{(customerName|orderId|status|trackingCode|trackingLine|storeName)\}/g, (_, key) => replacements[key] || '').replace(/\n{3,}/g, '\n\n').trim(); }
