...
app.get('/api/payments/pagarme/public-key', async (_req, res) => { const settings = await getPaymentsSettings(); return res.json({ ok: true, publicKey: settings.pagarme?.publicKey || process.env.PAGARME_PUBLIC_KEY || '' }); });
