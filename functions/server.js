...
const DEFAULT_SHIPPING_SETTINGS = { montagemPercent: 0.12, correios: { enabled: true, origemCep: process.env.LOJA_ORIGEM_CEP || '', servicos: String(process.env.CORREIOS_SERVICOS || '03298,03328').split(',').map(s => String(s).trim()).filter(Boolean), pesoKgPadrao: 1, alturaCmPadrao: 10, larguraCmPadrao: 15, comprimentoCmPadrao: 20, valorDeclaradoPadrao: 0, maxWeightKg: 30, maxDimensionCm: 100 }, businessRules: { arianaMoveis: { enabled: true, sellerNames: ['ARIANA MOVEIS', 'ARIANA MÓVEIS'], freeCepStart: '39740-000', freeCepEnd: '39740-000', label: 'Ariana Móveis', prazo: '1 a 3 dias úteis' }, snDigital: { enabled: true, sellerNames: ['SN DIGITAL', 'SN'], maxKmTier1: 40, priceTier1: 120, maxKmTier2: 70, priceTier2: 190, label: 'SN Digital', prazo: '1 a 3 dias úteis' }, rodocap: { enabled: true, sellerNames: ['SN DIGITAL', 'SN'], minKmExclusive: 70, percentOfInvoice: 0.12, label: 'Rodocap', prazoPadrao: 'sob consulta', allowedCities: RODOCAP_ALLOWED_CITIES, onlyUrbanArea: true } }, carriers: { correios: { enabled: true, maxWeightKg: 30, maxDimensionCm: 100 }, totalExpress: { enabled: true, maxWeightKg: 30, maxDimensionCm: 110 }, ownDelivery: { enabled: true, tiers: [{ maxKm: 30, price: 35 }, { maxKm: 60, price: 70 }] } } };
...
const STORE_DEFAULT_CEP = normalizeCepValue(process.env.LOJA_ORIGEM_CEP || '39740000') || '39740000';
...
async function lookupCepGeo(cep = '') {
  const normalizedCep = normalizeCepValue(cep);
  if (!normalizedCep) return null;
  if (cepGeoCache.has(normalizedCep)) return cepGeoCache.get(normalizedCep);
  const userAgent = 'ArianaMoveis/1.0 (shipping distance calculator)';
  try {
    const brasilApiUrl = `https://brasilapi.com.br/api/cep/v2/${normalizedCep}`;
    const response = await axios.get(brasilApiUrl, { timeout: 12000, headers: { 'User-Agent': userAgent } });
    const loc = response.data?.location?.coordinates || {};
    ...
    if (Number.isFinite(lat) && Number.isFinite(lon)) {
      ...
      cepGeoCache.set(normalizedCep, parsed);
      return parsed;
    }
  } catch (_error) {}
  try {
    ...
    const response = await axios.get(nominatimUrl, {
      ...
      params: { q: `${normalizedCep}, Brazil`, format: 'jsonv2', addressdetails: 1, countrycodes: 'br', limit: 1 }
    });
    ...
    if (Number.isFinite(lat) && Number.isFinite(lon)) {
      const parsed = {
        cep: normalizedCep,
        city: first?.address?.city || first?.address?.town || first?.address?.municipality || first?.address?.state_district || '',
        state: first?.address?.state || '',
        neighborhood: first?.address?.suburb || first?.address?.neighbourhood || '',
        latitude: lat,
        longitude: lon,
        source: 'nominatim'
      };
  ...
  cepGeoCache.set(normalizedCep, null);
  return null;
}
async function getDistanceKm(originCep, destinationCep) {
  const origin = await lookupCepGeo(originCep || STORE_DEFAULT_CEP);
  const destination = await lookupCepGeo(destinationCep);
...
async function resolveDestinationLocation(body = {}) { const cep = normalizeCepValue(body.cepDestino || body.cep || body.destinationCep || body.shippingAddress?.cep || ''); const explicitCity = body.cidade || body.city || body.destinationCity || body.shippingAddress?.cidade || body.shippingAddress?.city || ''; const explicitState = body.uf || body.state || body.destinationState || body.shippingAddress?.uf || body.shippingAddress?.state || ''; if (explicitCity) return { cep, city: String(explicitCity).trim(), state: String(explicitState || '').trim(), source: 'request' }; const geo = await lookupCepGeo(cep); if (geo) return { cep, city: geo.city || '', state: geo.state || '', neighborhood: geo.neighborhood || '', latitude: geo.latitude, longitude: geo.longitude, source: geo.source || 'geo' }; const viaCep = await lookupCepInfo(cep); if (viaCep) return { ...viaCep, source: 'viacep' }; return { cep, city: '', state: '', source: cep ? 'cep_only' : 'unknown' }; }
...
async function calculateShipping(body = {}) { const settings = await getShippingSettings(); const businessRules = settings.businessRules || {}; const arianaRule = businessRules.arianaMoveis || {}; const snRule = businessRules.snDigital || {}; const rodocapRule = businessRules.rodocap || {}; const weightKg = getBodyWeightKg(body, settings); const maxDimensionCm = getBodyMaxDimensionCm(body, settings); const productPrice = Number(body.productPrice || body.price || body.valorNota || body.invoiceValue || 0); const destinationCep = normalizeCepValue(body.cepDestino || body.cep || body.destinationCep || body.shippingAddress?.cep || ''); const sellerCtx = getSellerContext(body); const location = await resolveDestinationLocation(body); const requestedDistanceKm = Number(body.distanceKm || body.km || 0); const estimatedDistanceKm = requestedDistanceKm > 0 ? requestedDistanceKm : await getDistanceKm(settings?.correios?.origemCep || STORE_DEFAULT_CEP, destinationCep); const distanceKm = Number.isFinite(estimatedDistanceKm) ? Number(estimatedDistanceKm) : 0; const options = []; const isAriana = body.shippingRule === 'ariana' || sellerCtx.isAriana; const isSNDigital = body.shippingRule === 'sn_digital' || sellerCtx.isSNDigital; if (isAriana && arianaRule.enabled !== false && destinationCep && cepInRange(destinationCep, arianaRule.freeCepStart, arianaRule.freeCepEnd)) { options.push(buildManualShippingOption({ service: 'ariana_free_local', label: arianaRule.label || 'Ariana Móveis', price: 0, prazo: arianaRule.prazo || '1 a 3 dias úteis', provider: 'configured', details: `Frete grátis para o CEP ${arianaRule.freeCepStart}.`, metadata: { rule: 'ariana_free_local', cep: destinationCep } })); }
...
app.post('/api/payments/mp/pix', async (req, res) => { try { const body = req.body || {}; const payload = { transaction_amount: Number(body.amount || body.total || 0), description: body.description || `Pedido Ariana Móveis`, payment_method_id: 'pix', payer: { email: body.email || body.payer?.email || 'cliente@arianamoveis.com', first_name: body.first_name || body.payer?.first_name || 'Cliente', last_name: body.last_name || body.payer?.last_name || 'Ariana', identification: body.identification || body.payer?.identification || undefined }, metadata: { orderId: body.orderId || null }, notification_url: body.notification_url || `${APP_BASE_URL || 'http://localhost:3000'}/api/webhooks/mercadopago` }; const { response, idempotencyKey } = await createMercadoPagoPayment(payload); await writeAuditLog({ scope: 'payments', eventType: 'mercadopago_pix_created', orderId: body.orderId || null, status: response.status >= 200 && response.status < 300 ? 'success' : 'error', statusCode: response.status, request: payload, response: response.data, metadata: { provider: 'mercadopago', idempotencyKey } }); return res.status(response.status).json({ ok: response.status >= 200 && response.status < 300, data: response.data }); } catch (error) { return res.status(500).json({ ok: false, error: error.message || 'Erro ao criar PIX no Mercado Pago' }); } });
...
app.post('/api/payments/mp/boleto', async (req, res) => { try { const body = req.body || {}; const payload = { transaction_amount: Number(body.amount || body.total || 0), description: body.description || `Pedido Ariana Móveis`, payment_method_id: 'bolbradesco', payer: body.payer || {}, metadata: { orderId: body.orderId || null }, notification_url: body.notification_url || `${APP_BASE_URL || 'http://localhost:3000'}/api/webhooks/mercadopago` }; const { response, idempotencyKey } = await createMercadoPagoPayment(payload); await writeAuditLog({ scope: 'payments', eventType: 'mercadopago_boleto_created', orderId: body.orderId || null, status: response.status >= 200 && response.status < 300 ? 'success' : 'error', statusCode: response.status, request: payload, response: response.data, metadata: { provider: 'mercadopago', idempotencyKey } }); return res.status(response.status).json({ ok: response.status >= 200 && response.status < 300, data: response.data }); } catch (error) { return res.status(500).json({ ok: false, error: error.message || 'Erro ao criar boleto no Mercado Pago' }); } });
