...
app.post('/api/orders', async (req, res) => { try { const body = req.body || {}; const items = ensureArray(body.items).map((item) => { const qty = Number(item.qty || item.quantity || 1); const unitPrice = Number(item.unitPrice || item.price || 0); return ({ productId: String(item.productId || item._id || ''), sellerId: String(item.sellerId || ''), name: item.name || item.title || '', sku: item.sku || '', qty, unitPrice, totalPrice: Number(item.totalPrice || (unitPrice * qty)), image: item.image || item.imageUrl || '', imageUrl: item.imageUrl || item.image || '' }); }); const computedSubtotal = items.reduce((sum, item) => sum + Number(item.totalPrice || 0), 0); const subtotal = Number(body.subtotal || body.totals?.products || computedSubtotal || 0); const shippingCost = Number(body.shippingCost || body.shipping?.price || body.totals?.shipping || 0); const montagemCost = Number(body.montagemCost || body.totals?.montagem || 0); const total = Number(body.total || body.totals?.grandTotal || body.totals?.total || (subtotal + shippingCost + montagemCost)); const sellerIds = Array.from(new Set(items.map(item => item.sellerId).filter(Boolean))); const paymentMethod = body.paymentMethod || body.totals?.paymentMethod || body.payment?.method || body.payment?.type || ''; const payment = { ...(body.payment || {}), method: paymentMethod || body.payment?.method || '', type: body.payment?.type || paymentMethod || '', status: body.payment?.status || body.paymentStatus || body.status || 'pending_payment' }; const shippingAddress = body.shippingAddress || body.address || body.endereco || {}; const shipping = { ...(body.shipping || {}), price: Number(body.shipping?.price || shippingCost || 0) }; const order = await Order.create({ userId: normalizeObjectId(body.userId) || null, sellerIds, customerName: body.customerName || body.customer?.name || '', customerEmail: body.customerEmail || body.customer?.email || '', customerPhone: body.customerPhone || body.customer?.phone || body.phone || '', status: body.status || 'pendente', statusLabel: body.statusLabel || body.status || 'pendente', items, subtotal, shippingCost, montagemCost, total, payment, shippingAddress, shipping, notes: body.notes || '', manufacturer: body.manufacturer || sellerIds[0] || '' }); if (body.enqueueManufacturer !== false) await enqueueManufacturerDispatch(order); await Notification.create({ type: 'order_created', title: 'Novo pedido criado', message: `Pedido ${order._id} criado com sucesso.`, relatedId: String(order._id), severity: 'info' }); const normalizedOrder = { ...toJSON(order), address: shippingAddress, totals: { products: subtotal, shipping: shippingCost, grandTotal: total, grandTotalOriginal: total, paymentMethod: paymentMethod || '' } }; return res.json({ ok: true, id: String(order._id), orderId: String(order._id), orderNumber: `#AM-${String(order._id).slice(-6).toUpperCase()}`, item: normalizedOrder, order: normalizedOrder }); } catch (error) { return res.status(500).json({ ok: false, error: error.message || 'Erro ao criar pedido' }); } });
