...
const allowedOrigins = [
  'http://127.0.0.1:5500',
  'http://localhost:5500',
  'https://ariana-moveis-site.onrender.com'
];
...
async function calculateShipping(body = {}) { const settings = await getShippingSettings(); const weightKg = Number(body.weightKg || body.weight || settings.correios?.pesoKgPadrao || 0); const maxDimensionCm = Number(body.maxDimensionCm || body.dimensionCm || Math.max(Number(body.comprimento || body.comprimentoCm || settings.correios?.comprimentoCmPadrao || 0), Number(body.largura || body.larguraCm || settings.correios?.larguraCmPadrao || 0), Number(body.altura || body.alturaCm || settings.correios?.alturaCmPadrao || 0))); const distanceKm = Number(body.distanceKm || body.km || 0); const productPrice = Number(body.productPrice || body.price || 0); const options = []; const correios = settings.carriers?.correios || {}; if (correios.enabled) { try { const quoted = await quoteCorreios(body, settings); if (Array.isArray(quoted.quotes)) options.push(...quoted.quotes.map(q => ({ service: q.service, label: q.label || q.name || 'Correios', price: Number(q.price || 0), prazo: q.prazo || (q.deadlineDays ? `${q.deadlineDays} dia(s)` : null), provider: 'correios', raw: q.raw || null }))); } catch (error) { options.push({ service: 'correios_error', label: 'Correios', unavailable: true, error: error.message }); } } const totalExpress = settings.carriers?.totalExpress || {}; if (totalExpress.enabled && weightKg > 0 && weightKg <= Number(totalExpress.maxWeightKg || 30) && maxDimensionCm > 0 && maxDimensionCm <= Number(totalExpress.maxDimensionCm || 110)) { const base = Number(settings.totalExpressBasePrice || 0); if (base > 0) options.push({ service: 'total_express', label: 'Total Express', price: base, prazo: settings.totalExpressPrazo || 'sob consulta', provider: 'configured' }); } const ownDelivery = settings.carriers?.ownDelivery || {}; if (ownDelivery.enabled && Number(distanceKm || 0) > 0) { const own = calculateOwnDelivery(distanceKm, ownDelivery.tiers || []); if (own.available) options.push({ service: 'own_delivery', label: 'Entrega Própria', price: own.price, prazo: '1 a 3 dias úteis', provider: 'configured' }); } options.sort((a, b) => Number(a.price ?? 1e9) - Number(b.price ?? 1e9)); const cheapest = options.find(o => Number.isFinite(Number(o.price))) || null; const montagemCost = Number((productPrice * Number(settings.montagemPercent || 0.12)).toFixed(2)); return { ok: true, options, cheapest, montagemCost, settingsUsed: { montagemPercent: settings.montagemPercent, correios: settings.correios || {}, carriers: settings.carriers || {} } }; }
...
app.post('/api/admin/login', async (req, res) => {
  try {
    ...
    if (ADMIN_EMAIL && ADMIN_PASSWORD && email === ADMIN_EMAIL && password === ADMIN_PASSWORD) {
      adminUser = { id: 'env-admin', userId: 'env-admin', uid: 'env-admin', email, role: 'admin', admin: true, active: true, name: ADMIN_NAME };
    } else {
      const user = await User.findOne({ email });
      if (user && String(user.role || '').toLowerCase() === 'admin') {
        const valid = await bcrypt.compare(password, user.passwordHash || '');
        if (valid) adminUser = { id: String(user._id), userId: String(user._id), uid: String(user._id), email: user.email || email, role: 'admin', admin: true, active: user.isActive !== false, name: user.name || ADMIN_NAME };
      }
    }
    if (!adminUser) return res.status(401).json({ ok: false, error: 'invalid_admin_credentials' });
    const token = signAdminToken(adminUser);
    return res.json({ ok: true, token, ...adminUser, user: adminUser });
  } catch (error) {
