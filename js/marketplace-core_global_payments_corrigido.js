...
(function () {
  const toNumber = (v) => {
    // Aceita números, "1899,00", "1.899,00", "R$ 1.899,00"
    if (v === null || v === undefined) return null;
    if (typeof v === "number") return Number.isFinite(v) ? v : null;

    // Se existir um parser global (marketplace-core), usa ele
    try {
      if (typeof window !== "undefined" && typeof window.__toNumberBR === "function") {
        const n0 = window.__toNumberBR(v);
    ...
    let clean = s.replace(/R\$\s?/gi, "").replace(/\s+/g, "");
  ...
  const buildInstallmentsText = (inst) => {
    if (!inst?.count || !(inst?.value > 0)) return "em até 10x";
